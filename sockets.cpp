...
// test the server with:  telnet localhost 1234
// also based on:  http://shoe.bocks.com/net/#top

int main(int argc, char *argv[])
{
	// variables declaration
	int master_socket, addrlen, new_socket, bufsize = 1025, reuse=TRUE,
		max_clients = 3, activity, loop1, loop2, errno;
	...
	char *message= (char*) malloc (26 * sizeof(char)), 
		 ...
		 *ipAddress = getIpAddr((char*)"eth1");
	fd_set readfds;
	
	if (argc > 1)
		memcpy(originalPort, argv[1], 4*sizeof(char));
	else 
    ...
    /// in my machine, the wireless interface is eth1. yours might differ
    if ((errno = getaddrinfo(ipAddress, originalPort, &hints, &servinfo)) != 0) 
    {   fprintf(stderr, "getaddrinfo error: %s\n", gai_strerror(errno)); exit(1);  }
